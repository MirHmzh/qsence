...
function initCam(){const qrcodes=window.qrcode;const video=document.createElement("video");const canvasElement=document.getElementById("qri");const canvas=canvasElement.getContext("2d");const qrResult=document.getElementById("qr-result");const outputData=document.getElementById("outputData");let scanning=!1;qrcodes.callback=(res)=>{if(res){alerting()}};try{navigator.mediaDevices.getUserMedia({video:{facingMode:"environment"}}).then(function(stream){scanning=!0;canvasElement.hidden=!1;video.setAttribute("playsinline",!0);video.srcObject=stream;video.play();vids=stream;tick();scan()});alerting()}catch(e){alerting()}
